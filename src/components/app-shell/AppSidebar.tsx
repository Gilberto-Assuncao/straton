...
export function AppSidebar({ navigation, collapsed, mobile, onClose }: { navigation:AppNavigationItem[]; collapsed:boolean; mobile?:boolean; onClose?:()=>void }) {
  ...
  return <aside id={mobile?"mobile-app-navigation":undefined} aria-label={tShell("primaryNavigation")} className={`flex h-full flex-col border-r border-edge-10 bg-surface-alt transition-[width] ${mobile?"w-[min(20rem,90vw)]":collapsed?"w-20":"w-72"}`}><div className="flex min-h-20 items-center justify-between border-b border-edge-10 px-4"><Logo compact={collapsed}/>{mobile?<button type="button" onClick={onClose} aria-label={tShell("closeNavigation")} className="min-h-11 min-w-11 rounded-lg text-ink focus-visible:outline-2 focus-visible:outline-brand-bright">×</button>:null}</div><nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3">
