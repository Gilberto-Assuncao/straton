...
export default async function DashboardPage() {
  ...
  return (
    <section aria-labelledby="dashboard-heading">
      <PageHeader headingId="dashboard-heading" eyebrow={t("eyebrow")} title={t("welcome", { name: firstName })} description={t("description")} actions={<><Link href="/dashboard/time" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-brand px-4 text-sm font-semibold text-on-brand transition hover:bg-brand-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-bright">{t("addTimeEntry")}</Link><Link href="/dashboard/employees/new" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-edge-15 px-4 text-sm font-semibold text-ink transition hover:bg-edge-5 focus-visible:outline-2 focus-visible:outline-brand-bright">{t("inviteEmployee")}</Link></>} />
