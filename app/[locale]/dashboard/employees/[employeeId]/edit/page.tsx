...
export default async function EditEmployeePage({ params }: { params: Promise<{ employeeId: string }> }) {
  ...
  return (
    <section aria-labelledby="edit-employee-heading">
      <Link href={`/dashboard/employees/${employee.id}`} className="inline-flex min-h-11 items-center text-sm font-semibold text-ink-muted hover:text-ink focus-visible:outline-2 focus-visible:outline-brand-bright">← {t("backToProfile")}</Link>
