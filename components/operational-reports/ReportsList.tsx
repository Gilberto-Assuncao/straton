...
export default async function ReportsList({ reports }: { reports: OperationalReportListItem[] }) {
  ...
  if (reports.length === 0) {
    return <div className="rounded-2xl border border-dashed border-edge-15 bg-surface p-8 text-center text-sm text-ink-muted">{t("noReports")}</div>;
  }
  return (
          ...
          <tbody className="divide-y divide-edge-10">
            {reports.map((report) => (
              <tr key={report.id} className="hover:bg-white/[0.03]">
                ...
                <td className="px-5 py-4">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusStyles[report.status] ?? "bg-edge-10 text-ink-muted"}`}>{t(`status_${report.status}` as "status_draft")}</span>
                </td>
              </tr>
            ))}
          </tbody>
