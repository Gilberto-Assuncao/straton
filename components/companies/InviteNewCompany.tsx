...
export default function InviteNewCompany({ invites }: { invites: CompanyInvite[] }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#161A34] p-5 sm:p-6">
      <h3 className="text-lg font-semibold text-[#E5E7EB]">{t("title")}</h3>
      <p className="mt-1 text-sm text-[#9CA3AF]">{t("subtitle")}</p>
      ...
      <form action={submit} className="mt-5 grid gap-4">
        <div className="relative grid gap-1.5">
          ...
          {suggestions.length > 0 ? (
            <ul className="absolute top-full z-10 mt-1 w-full rounded-lg border border-white/10 bg-[#111827] p-1 shadow-xl">
              {suggestions.map((suggestion) => (
                <li key={suggestion.enterpriseNumber}>
                  <button
                    type="button"
                    onClick={() => choose(suggestion)}
                    className="flex w-full flex-col items-start rounded-md px-3 py-2 text-left text-sm text-[#E5E7EB] hover:bg-white/5"
                  >
        ...
        <button
          type="submit"
          disabled={pending}
          className="min-h-11 justify-self-start rounded-lg bg-[#22C55E] px-5 text-sm font-semibold text-[#06121F] disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-[#22C55E]"
        >
      ...
      {result ? (
        <div className="mt-4">
          <p role="status" className={`text-sm ${result.ok ? "text-[#4ADE80]" : "text-red-300"}`}>
            {t(`message_${result.key}` as "message_sent")}
          </p>
          {result.link ? (
            <div className="mt-3 rounded-xl border border-white/10 bg-[#111C33] p-3">
              <p className="text-xs text-[#9CA3AF]">{t("linkHint")}</p>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <code className="min-w-0 flex-1 truncate rounded bg-black/30 px-2 py-1 text-xs text-[#D1D5DB]">
                  {result.link}
                </code>
                <button
                  type="button"
                  ...
                  className="min-h-11 rounded-lg border border-white/15 px-4 text-xs font-semibold text-[#E5E7EB] hover:bg-white/5"
                >
      ...
      {open.length > 0 ? (
        <div className="mt-6">
          <p className="text-sm font-semibold uppercase tracking-wide text-[#6B7280]">{t("pendingTitle")}</p>
          <ul className="mt-3 space-y-2">
            {open.map((invite) => (
              <li key={invite.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-white/[0.03] px-4 py-3">
                <div className="min-w-0">
                  <p className="font-medium text-[#E5E7EB]">{invite.companyName}</p>
                ...
                <button
                  ...
                  disabled={pending}
                  className="min-h-11 rounded-lg border border-white/15 px-4 text-xs font-semibold text-[#E5E7EB] hover:bg-white/5 disabled:opacity-50"
                >
                  {t("revoke")}
                </button>
              </li>
