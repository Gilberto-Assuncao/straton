...
export default async function RecentTimesheets({ timesheets }: { timesheets: RecentTimesheet[] }) {
  ...
  return (
    <section aria-labelledby="recent-timesheets-title" className="overflow-hidden rounded-2xl border border-white/10 bg-surface">
      ...
      {timesheets.length === 0 ? <p className="border-t border-white/10 px-5 py-12 text-center text-sm text-ink-muted">{t("recentTimesheetsEmpty")}</p> : <>
        <div className="hidden md:block"><table className="w-full border-collapse text-left text-sm"><thead className="border-y border-white/10 bg-surface-alt/60 text-xs uppercase tracking-wide text-ink-muted"><tr>{columns.map((heading) => <th key={heading} scope="col" className="px-6 py-4 font-medium">{heading}</th>)}</tr></thead><tbody className="divide-y divide-white/10">{timesheets.map((item) => <tr key={item.id} className="transition hover:bg-white/[0.03]"><th scope="row" className="px-6 py-4 font-semibold text-ink">{item.employee}</th><td className="px-6 py-4 text-ink-soft">{item.location}</td><td className="px-6 py-4 text-ink-soft">{item.hours}h</td><td className="px-6 py-4 text-ink-muted">{item.date}</td><td className="px-6 py-4"><StatusBadge status={item.status} /></td></tr>)}</tbody></table></div>
