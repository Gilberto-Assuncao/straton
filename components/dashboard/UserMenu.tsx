...
export default async function UserMenu() {
  ...
  return (
    <details className="group relative">
      <summary aria-label="Open user menu" className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-lg px-1 focus-visible:outline-2 focus-visible:outline-brand-bright [&::-webkit-details-marker]:hidden">
      ...
      <div className="absolute right-0 top-12 z-40 w-52 rounded-xl border border-edge-10 bg-surface p-2 shadow-2xl">
        ...
        <button type="button" className="mt-1 flex min-h-11 w-full items-center rounded-lg px-3 text-sm text-ink-muted hover:bg-edge-5 hover:text-ink focus-visible:outline-2 focus-visible:outline-brand-bright">Sign Out</button>
      </div>
    </details>
  );
}
