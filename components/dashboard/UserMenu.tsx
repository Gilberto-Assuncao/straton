...
export default async function UserMenu() {
  ...
  return (
    <details className="group relative">
      ...
      <div className="absolute right-0 top-12 z-40 w-52 rounded-xl border border-white/10 bg-surface p-2 shadow-2xl">
        <div className="border-b border-white/10 px-3 py-2"><p className="text-sm font-semibold text-ink">{user.name}</p><p className="text-xs text-ink-muted">{role}</p></div>
        <button type="button" className="mt-1 flex min-h-11 w-full items-center rounded-lg px-3 text-sm text-ink-muted hover:bg-white/5 hover:text-ink focus-visible:outline-2 focus-visible:outline-brand">Sign Out</button>
      </div>
    </details>
  );
}
