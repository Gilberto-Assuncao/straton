...
export default function EmployeeTable({ employees, teams }: { employees: Employee[]; teams: string[] }) {
  ...
  return <div className="mt-6"><EmployeeFilters search={search} status={status} team={team} teams={teams} onSearch={setSearch} onStatus={setStatus} onTeam={setTeam} onClear={clear} hasFilters={hasFilters} /><p className="mt-4 text-sm text-ink-muted" aria-live="polite">{t("showingCount", { shown: filtered.length, total: employees.length })}</p>{filtered.length === 0 ? <div className="mt-4"><EmptyEmployeesState filtered={hasFilters} /></div> : <><div className="mt-4 hidden overflow-hidden rounded-2xl border border-white/10 bg-surface lg:block"><table className="w-full border-collapse text-left text-sm"><thead className="border-b border-white/10 bg-surface-alt/60 text-xs uppercase tracking-wide text-ink-muted"><tr>{columns.map((heading) => <th key={heading} scope="col" className="px-5 py-4 font-medium">{heading}</th>)}</tr></thead><tbody className="divide-y divide-white/10">{filtered.map((employee) => <tr key={employee.id} className="hover:bg-white/[0.03]"><th scope="row" className="px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-brand/10 text-xs font-bold text-brand">{employee.avatarInitials}</span><div><p className="font-semibold text-ink">{employee.firstName} {employee.lastName}</p><p className="text-xs font-normal text-ink-muted">{employee.email}</p></div></div></th><td className="px-5 py-4 text-ink-soft">{employee.jobTitle}</td><td className="px-5 py-4 text-ink-soft">{employee.team ?? t("noTeam")}</td><td className="px-5 py-4"><EmployeeStatusBadge status={employee.status} /></td><td className="px-5 py-4 text-ink-soft">{employee.totalHoursThisWeek}h</td><td className="px-5 py-4 text-ink-muted">{employee.startDate}</td><td className="px-5 py-4"><div className="flex gap-1"><Link href={`/dashboard/employees/${employee.id}`} className="flex min-h-11 items-center px-2 font-semibold text-brand focus-visible:outline-2 focus-visible:outline-brand">{t("view")}</Link><Link href={`/dashboard/employees/${employee.id}/edit`} className="flex min-h-11 items-center px-2 font-semibold text-ink-soft focus-visible:outline-2 focus-visible:outline-brand">{t("edit")}</Link><EmployeeStatusButton employeeId={employee.id} status={employee.status} /></div></td></tr>)}</tbody></table></div><ul className="mt-4 grid gap-4 lg:hidden">{filtered.map((employee) => <EmployeeCard key={employee.id} employee={employee} />)}</ul></> }</div>;
}
