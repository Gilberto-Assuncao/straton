    ...
    <section aria-labelledby="worked-hours-heading" className="grid gap-5">
      <div className={card}>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 id="worked-hours-heading" className="text-lg font-semibold text-ink">
              {t("workedHoursTitle")}
            </h2>
            <p className="mt-1 text-sm text-ink-muted">{t("workedHoursSubtitle")}</p>
          </div>
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            */}
            <label className="grid gap-1.5">
              <span className="text-xs text-ink-muted">{t("monthLabel")}</span>
            ...
            <a
              href={csvHref}
              className="flex min-h-11 items-center rounded-lg border border-edge-15 px-4 text-sm font-semibold text-ink hover:bg-edge-5 focus-visible:outline-2 focus-visible:outline-brand-bright"
            >
              {t("exportCsv")}
            </a>
          </div>
        ...
        */}
        {locations.length > 1 ? (
          <fieldset className="mt-5 border-0 p-0">
            <legend className="text-xs text-ink-muted">{t("locationsLabel")}</legend>
        ...
        */}
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-brand/30 bg-brand/[0.06] p-4">
            <p className="text-xs text-ink-muted">{t("approved")}</p>
            <p className="mt-2 font-mono text-[28px] font-bold text-brand-bright">{formatMinutes(report.totals.approvedMinutes)}</p>
            <p className="mt-1 text-xs text-ink-subtle">{t("approvedHint")}</p>
          </div>
      ...
      <div className={card}>
        {report.people.length === 0 ? (
          <p className="py-8 text-center text-sm text-ink-subtle">{t("noData")}</p>
        ) : (
              ...
              <tbody>
                {report.people.map((person) => (
                  <tr key={person.membershipId} className="border-b border-edge-5">
                    ...
                    <td className={`${cell} text-right font-mono text-ink-muted`}>{person.entryCount}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
      ...
      */}
      {hasDividedLocation ? (
        <div className={card}>
          <h3 className="text-sm font-semibold text-ink">{t("areasTitle")}</h3>
          <p className="mt-1 text-xs text-ink-subtle">{t("areasSubtitle")}</p>
      ...
      {report.sites.length > 0 ? (
        <div className={card}>
          <h3 className="text-sm font-semibold text-ink">{t("sitesTitle")}</h3>
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[32rem] border-collapse text-left">
              <thead>
                <tr className="border-b border-edge-10 text-xs uppercase tracking-wide text-ink-subtle">
