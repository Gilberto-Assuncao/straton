...
export default function HoursDivergenceReport({ teams, divergence, siteHours }: Props) {
  ...
  return <div className="grid gap-5">
    ...
    <section aria-labelledby="divergence-title" className="rounded-2xl border border-edge-10 bg-surface p-5 sm:p-6">
      ...
      {divergence.length ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm">
        <thead><tr className="border-b border-edge-10 text-xs uppercase tracking-wide text-ink-muted">
        ...
        <tbody>{divergence.map((row) => <tr key={row.membershipId} className="border-b border-edge-5 text-ink">
    ...
    <section aria-labelledby="site-hours-title" className="rounded-2xl border border-edge-10 bg-surface p-5 sm:p-6">
      <h2 id="site-hours-title" className="text-lg font-semibold text-ink">Hours by site — this week</h2>
      {siteHours.length ? <ul className="mt-4 grid gap-2">{siteHours.map((row) => <li key={row.site} className="flex items-center justify-between rounded-lg bg-edge-5 px-4 py-2 text-sm"><span className="text-ink">{row.site}</span><span className="font-semibold text-ink-muted">{row.hours}h</span></li>)}</ul>
