...
export default function PayrollSummary({ summary }: { summary: PayrollPeriodSummary }) {
  ...
  return (
    <div className="grid min-w-0 gap-5">
      <div className="rounded-2xl border border-amber-400/25 bg-amber-400/5 p-4 text-sm text-ink">
        <p className="font-semibold text-warning">{t("previewNoticeTitle")}</p>
        <p className="mt-1 text-ink-muted">{t("previewNoticeBody")}</p>
      </div>
          ...
          <div className="divide-y divide-white/5">
            ...
            {summary.employees.map((employee) => (
              <div key={employee.employee} className="grid grid-cols-4 gap-2 px-4.5 py-3 text-sm">
                <span className="font-semibold text-ink">{employee.employee}</span>
                <span className="font-mono text-ink-muted">{hours(employee.regularMinutes)}</span>
                <span className="font-mono text-ink-muted">{hours(employee.nightMinutes)}</span>
                <span className="font-mono font-semibold text-ink">{hours(employee.totalMinutes)}</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="p-4.5 text-sm text-ink-muted">{t("emptyState")}</p>
        )}
      </div>
    </div>
  );
