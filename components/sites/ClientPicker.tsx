...
import {
  ...
  createIndividualClientAction,
  searchClientSuggestionsAction,
  type CompanySuggestion,
} from "@/src/features/sites/actions";
import type { ClientKind, ClientOption } from "@/src/features/sites/types";
 ...
 *
 * A client is a person or a company (#85), and the choice comes first because
 * it changes everything after it. A company is searched in the Belgian register
 * by name, which is what the person filling this in actually knows. A private
 * client has no enterprise number to look up and never will, so asking for one
 * — or searching a register that cannot contain them — is asking somebody to
 * answer a question about their customer that has no answer.
 */
export default function ClientPicker({ clients, defaultValue }: { clients: ClientOption[]; defaultValue?: string | null }) {
  ...
  const [kind, setKind] = useState<ClientKind>("company");
  const [person, setPerson] = useState({ name: "", email: "", phone: "", city: "" });
  ...
  /** Appends and selects whatever was just created, so the form can be
      submitted without a round trip to reload the list. */
  function adopt(id: string, name: string, newKind: ClientKind, city: string | null) {
    setOptions((current) => [...current, { id, name, kind: newKind, city }].sort((a, b) => a.name.localeCompare(b.name)));
    setSelected(id);
    setCreating(false);
    setSuggestions(null);
    setQuery("");
    setPerson({ name: "", email: "", phone: "", city: "" });
  }
  ...
  function add(input: { displayName: string; registrationNumber?: string; city?: string; postalCode?: string }) {
    ...
    startTransition(async () => {
      ...
      adopt(result.id, result.name, "company", input.city ?? null);
    });
  }
  ...
  function addPerson() {
    setError(null);
    startTransition(async () => {
      const result = await createIndividualClientAction(person);
      if (!result.ok) {
        setError(result.messageKey);
        return;
      }
      adopt(result.id, result.name, "individual", person.city.trim() || null);
    });
  }
    ...
    <div className="sm:col-span-2">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <div className="min-w-0 flex-1">
          <select id="site-client" name="clientId" value={selected} onChange={(event) => setSelected(event.target.value)} className={field}>
      ...
      {creating ? (
        <div className="mt-4 rounded-xl border border-edge-10 bg-surface-inset p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-ink-muted">{kind === "company" ? t("newClientHelp") : t("newClientHelpPerson")}</p>

          {/*
            The choice that comes first, because it changes the questions after
            it. A person has no register entry to search and no enterprise
            number to give.
          */}
          <fieldset className="mt-4">
            <legend className="text-sm font-medium text-ink">{t("clientKindLabel")}</legend>
            <div className="mt-2 flex flex-wrap gap-2">
              {(["company", "individual"] as const).map((option) => (
                <button
                  ...
                  onClick={() => { setKind(option); setError(null); }}
                  className={`min-h-11 rounded-lg border px-4 text-sm font-semibold transition focus-visible:outline-2 focus-visible:outline-brand ${
                    kind === option ? "border-brand/40 bg-brand/10 text-brand-bright" : "border-edge-15 text-ink hover:bg-edge-5"
                  }`}
                >
                  {option === "company" ? t("clientKindCompany") : t("clientKindIndividual")}
                </button>
              ))}
            </div>
          </fieldset>
          ...
          {kind === "company" ? (
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <label htmlFor="person-name" className={label}>{t("personNameLabel")}</label>
                <input
                  id="person-name"
                  value={person.name}
              ...
              <div>
                <label htmlFor="person-phone" className={label}>{t("personPhoneLabel")}</label>
                <input id="person-phone" type="tel" value={person.phone} onChange={(event) => setPerson((current) => ({ ...current, phone: event.target.value }))} className={field} />
              </div>
              <div>
                <label htmlFor="person-email" className={label}>{t("personEmailLabel")}</label>
                <input id="person-email" type="email" value={person.email} onChange={(event) => setPerson((current) => ({ ...current, email: event.target.value }))} className={field} />
              </div>
              <div>
                <label htmlFor="person-city" className={label}>{t("personCityLabel")}</label>
                <input id="person-city" value={person.city} onChange={(event) => setPerson((current) => ({ ...current, city: event.target.value }))} className={field} />
              </div>
              <div className="flex items-end">
                <button
                  type="button"
                  onClick={addPerson}
                  disabled={pending || person.name.trim().length < 2}
                  className="mt-2 min-h-12 w-full rounded-lg border border-brand/40 bg-brand/10 px-4 text-sm font-semibold text-brand-bright transition hover:bg-brand/20 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {t("personAdd")}
                </button>
              </div>
