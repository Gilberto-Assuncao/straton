...
export default function AuthStatus({ state }: { state: AuthActionState }) {
  ...
  return (
    ...
    <p
      id="auth-status"
      role={state.status === "error" ? "alert" : "status"} className={`rounded-lg border px-4 py-3 text-sm ${state.status === "error" ? "border-red-400/30 bg-red-400/10 text-danger-soft" : "border-brand/30 bg-brand/10 text-green-100"}`}>
      {t(state.messageKey)}
    </p>
  );
}
