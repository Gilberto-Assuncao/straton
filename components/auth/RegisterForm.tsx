...
export default function RegisterForm() {
  ...
  return (
    <AuthCard action={action} title={t("registerTitle")} description={t("registerDescription")} footer={<>{t("alreadyHaveAccount")} <Link href="/login" className="inline-flex min-h-11 items-center font-semibold text-[#22C55E] hover:text-[#16A34A] focus-visible:outline-2 focus-visible:outline-[#22C55E]">{t("signIn")}</Link></>}>
      <fieldset><legend className="mb-2 text-sm font-medium text-[#E5E7EB]">{t("accountType")}</legend><div key={formKey} className="grid grid-cols-2 gap-3">{([["individual", t("accountIndividual")], ["company", t("accountCompany")]] as const).map(([value, type]) => <label key={value} className="flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-lg border border-white/15 bg-[#111827] px-3 text-sm font-medium has-checked:border-[#22C55E] has-checked:bg-[#22C55E]/10"><input type="radio" name="accountType" value={value} onInput={onInput} defaultChecked={value === chosenType} className="h-4 w-4 accent-[#22C55E]" />{type}</label>)}</div></fieldset>
