...
export default function RegisterForm() {
  ...
  return (
    <AuthCard action={action} title={t("registerTitle")} description={t("registerDescription")} footer={<>{t("alreadyHaveAccount")} <Link href="/login" className="inline-flex min-h-11 items-center font-semibold text-brand hover:text-brand-hover focus-visible:outline-2 focus-visible:outline-brand">{t("signIn")}</Link></>}>
      <fieldset><legend className="mb-2 text-sm font-medium text-ink">{t("accountType")}</legend><div key={formKey} className="grid grid-cols-2 gap-3">{([["individual", t("accountIndividual")], ["company", t("accountCompany")]] as const).map(([value, type]) => <label key={value} className="flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-lg border border-white/15 bg-surface-alt px-3 text-sm font-medium has-checked:border-brand has-checked:bg-brand/10"><input type="radio" name="accountType" value={value} onInput={onInput} defaultChecked={value === chosenType} className="h-4 w-4 accent-brand" />{type}</label>)}</div></fieldset>
      ...
      <label className="flex cursor-pointer items-start gap-3 text-sm leading-6 text-ink-soft"><input type="checkbox" name="terms" required className="mt-1 h-5 w-5 shrink-0 rounded border-white/20 accent-brand" /><span>{t("acceptPrefix")} <a href="#" className="font-medium text-brand hover:text-brand-hover">{t("termsOfService")}</a> {t("and")} <a href="#" className="font-medium text-brand hover:text-brand-hover">{t("privacyPolicy")}</a>.</span></label>
