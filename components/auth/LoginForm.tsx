...
export default function LoginForm({
  ...
  return (
    <AuthCard action={action} title={t("title")} description={t("description")} footer={<>{t("newToStraton")} <Link href="/register" className="inline-flex min-h-11 items-center font-semibold text-brand-bright hover:text-brand-hover focus-visible:outline-2 focus-visible:outline-brand-bright">{t("createAccount")}</Link></>}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
        ...
        <Link href="/forgot-password" className="inline-flex min-h-11 items-center font-medium text-brand-bright hover:text-brand-hover focus-visible:outline-2 focus-visible:outline-brand-bright">{t("forgotPassword")}</Link>
      </div>
